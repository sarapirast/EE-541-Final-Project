...
class BiLSTMBaseline(nn.Module):
    """
    Single-layer bidirectional LSTM baseline.
    Concatenates final forward & backward hidden states and applies FC + dropout.
    """
    ...
    def forward(self, x):
        ...
        h_forward = h_n[0]  # first layer, forward
        h_backward = h_n[1] # first layer, backward
...
def main():
    ...
    model = BiLSTMBaseline(vocab_size, embed_dim, hidden_dim, num_classes).to(device)
    ...
    sns.heatmap(cm, annot=True, fmt="d", cmap="Blues", xticklabels=label_id_to_string.values(),
                yticklabels=label_id_to_string.values())
